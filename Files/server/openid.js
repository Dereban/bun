...
passport.use(new OpenIDStrategy({
        ...
        stateless: true,
        returnURL: `${config.PERSONA_AUDIENCE}/auth/openid/return`,
        realm: config.PERSONA_AUDIENCE,
        profile: false
    },
    function(identifier, done) {
        var id = identifier.match(/\d+$/)[0];
        var auth = _.find(['admin', 'moderator', 'janitor', 'dj'], type =>
        config.staff[type] && id in config.staff[type])
        if (!auth) {
            var winston = require('winston');
            winston.error("Login attempt by " + id);
            return done(null,false,{message: 'bad openid'});
        }
        var user={id,auth};
