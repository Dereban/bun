...
const _ = require('underscore'),
	...
	passport = require ('../openid').passport,
...
app.use(function(req, res, next) {
	...
	const loginCookie = openid.extract_login_cookie(req.cookies);
	if (loginCookie) {
		openid.check_cookie(loginCookie, function (err, ident) {
			if (!err)
...
if (config.GZIP)
	app.use(compress());
app.post('/logout', openid.logout)
	.post('/upload/', imager.new_upload)
    .get('/auth/openid', passport.authenticate("openid",{failureRedirect:'/'}),function(req,res){res.redirect('/');})
    .get('/auth/openid/return', passport.authenticate('openid',{failureRedirect:'/'}),function(req,res){openid.set_cookie(res,req.user);})
	.use(admin)
	.use('/api/', api);
